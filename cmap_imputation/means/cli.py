...
def zero_one(v):
    try:
        if 0 - 1e-8 <= float(v) <= 1 + 1e-8:
            return float(v)
        else:
            raise argparse.ArgumentTypeError(f'Expected value in [0, 1], got {float(v)}')
    except ValueError:
        raise argparse.ArgumentTypeError(f'Expected value in [0, 1], got {v}')
...
def cli():
    ...
    options.add_argument('-f', dest='factor', default=0.5, type=zero_one,
                         help='Scale factor applied to each type of mean: 0 denotes\
                         mean over drug types, 1 denotes mean over cell types - Default 0.5')
...
def validate_inputs():
    ...
    if args.kfold > 0:
        ...
        path_prefix = f"MeansScaleFactor{args.factor}KFold{args.kfold}Folds"
    else:
        method = ('sparse', args.sparse_subset)
        path_prefix = f"MeansScaleFactor{args.factor}SparseSubset{args.sparse_subset}DrugsInTrain"
    ...
    # For cells to only be included in the training set, exclude them from allData
    ...
    return allData, only_train, method, args.seed, os.path.join(args.preds_path, path_prefix), args.factor, args.plot
